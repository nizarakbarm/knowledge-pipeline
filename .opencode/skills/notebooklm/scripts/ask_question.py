...
def ask_notebooklm(question: str, notebook_url: str, headless: bool = True) -> str:
    ...
    try:
        ...
        # Mark all pre-existing response elements so we can ignore them later.
        # NotebookLM reuses DOM nodes and loads old chat lazily, so element
        # counting doesn't work — we tag existing ones with an attribute.
        page.evaluate("""(selectors) => {
            selectors.forEach(s => {
                document.querySelectorAll(s).forEach(el => el.setAttribute('data-preexisting', 'true'));
            });
        }""", RESPONSE_SELECTORS)
        ...
        while time.time() < deadline:
            ...
            # Find only NEW elements (without data-preexisting attribute)
            try:
                # Build a combined selector that matches any RESPONSE_SELECTOR
                # that does NOT have the data-preexisting attribute.
                fresh_selector = ', '.join(
                    f'{s}:not([data-preexisting])' for s in RESPONSE_SELECTORS
                )
                fresh_elements = page.query_selector_all(fresh_selector)
                if fresh_elements:
                    latest = fresh_elements[-1]
